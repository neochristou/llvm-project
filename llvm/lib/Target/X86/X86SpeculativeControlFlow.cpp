...
class X86SpeculativeControlFlowPass : public MachineFunctionPass {
public:
...
char X86SpeculativeControlFlowPass::ID = 0;

void X86SpeculativeControlFlowPass::getAnalysisUsage(
...
bool X86SpeculativeControlFlowPass::runOnMachineFunction(
    MachineFunction &MF) {
  ...
  if (!EnableSpeculativeControlFlow)
    return false;
...
void X86SpeculativeControlFlowPass::hardenEdgesWithLFENCE(
    MachineFunction &MF) {
...
SmallVector<X86SpeculativeControlFlowPass::BlockCondInfo, 16>
X86SpeculativeControlFlowPass::collectBlockCondInfo(MachineFunction &MF) {
...
/// complete.
SmallVector<MachineInstr *, 16>
X86SpeculativeControlFlowPass::tracePredStateThroughCFG(
...
void X86SpeculativeControlFlowPass::unfoldCallAndJumpLoads(
    MachineFunction &MF) {
  for (MachineBasicBlock &MBB : MF)
...
SmallVector<MachineInstr *, 16>
X86SpeculativeControlFlowPass::tracePredStateThroughIndirectBranches(
    MachineFunction &MF) {
...
void X86SpeculativeControlFlowPass::tracePredStateThroughBlocksAndHarden(
    MachineFunction &MF) {
...
unsigned X86SpeculativeControlFlowPass::saveEFLAGS(
...
/// reliably lower.
void X86SpeculativeControlFlowPass::restoreEFLAGS(
...
void X86SpeculativeControlFlowPass::mergePredStateIntoSP(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator InsertPt, DebugLoc Loc,
    unsigned PredStateReg) {
...
unsigned X86SpeculativeControlFlowPass::extractPredStateFromSP(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator InsertPt,
    DebugLoc Loc) {
...
void X86SpeculativeControlFlowPass::hardenLoadAddr(
...
MachineInstr *X86SpeculativeControlFlowPass::sinkPostLoadHardenedInst(
...
bool X86SpeculativeControlFlowPass::canHardenRegister(unsigned Reg) {
...
unsigned X86SpeculativeControlFlowPass::hardenValueInRegister(
...
unsigned X86SpeculativeControlFlowPass::hardenPostLoad(MachineInstr &MI) {
  MachineBasicBlock &MBB = *MI.getParent();
  DebugLoc Loc = MI.getDebugLoc();
...
void X86SpeculativeControlFlowPass::hardenReturnInstr(MachineInstr &MI) {
  MachineBasicBlock &MBB = *MI.getParent();
  DebugLoc Loc = MI.getDebugLoc();
  auto InsertPt = MI.getIterator();
...
/// state.
void X86SpeculativeControlFlowPass::tracePredStateThroughCall(
    MachineInstr &MI) {
  MachineBasicBlock &MBB = *MI.getParent();
  MachineFunction &MF = *MBB.getParent();
  auto InsertPt = MI.getIterator();
...
void X86SpeculativeControlFlowPass::hardenIndirectCallOrJumpInstr(
    MachineInstr &MI,
    SmallDenseMap<unsigned, unsigned, 32> &AddrRegToHardenedReg) {
  switch (MI.getOpcode()) {
  case X86::FARCALL16m:
...
INITIALIZE_PASS_BEGIN(X86SpeculativeControlFlowPass, PASS_KEY,
                      "X86 speculative load hardener", false, false)
INITIALIZE_PASS_END(X86SpeculativeControlFlowPass, PASS_KEY,
                    "X86 speculative load hardener", false, false)

FunctionPass *llvm::createX86SpeculativeControlFlowPass() {
  return new X86SpeculativeControlFlowPass();
}
